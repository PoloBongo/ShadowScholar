...
public class LoadManager : MonoBehaviour
{
            ...
            if (jsonFile != null)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (jsonFile.shadowScholar.missions.isStart && !jsonFile.shadowScholar.missions.mission4.isFinish)
                            {
                                StartCoroutine(PreloadSceneAndAssetsMission4("Mission4"));
                            }
                            else
                            {
                                if (jsonFile.shadowScholar.missions.isStart && !jsonFile.shadowScholar.missions.mission5.isFinish)
                                {
                                    StartCoroutine(PreloadSceneAndAssetsMission5("Mission5"));
                                }
                                else
                                {
    ...
    #endregion
    #region LoadSceneMission5

    IEnumerator PreloadSceneAndAssetsMission5(string sceneName)
    {
        sceneIndex = SceneManager.GetActiveScene().buildIndex;
        loadingSlider.gameObject.SetActive(true);
        loadingText.color = new Color32(0xFF, 0xC2, 0x00, 0xFF);
